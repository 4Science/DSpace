...
public class OpenPolicyFinderService {
                ...
                try (CloseableHttpResponse response = client.execute(method)) {
                    ...
                    if (null != responseBody) {
                        ...
                        try {
                            ...
                            opfResponse = new OpenPolicyFinderPublisherResponse(content,
    ...
    public int performCountRequest(String type, String field, String predicate, String value) {
        // API Key is *required* for API calls
        if (null == apiKey) {
            log.error("Open Policy Finder API Key missing: please register for an API key and set" +
                      " openpolicyfinder.apikey");
            return 0;
        }
        ...
        try (CloseableHttpClient client = DSpaceHttpClientFactory.getInstance().buildWithoutAutomaticRetries(5)) {
            ...
            log.error("An error occurs counting the Open Policy Finder entries", ex);
            return 0;
        } finally {
            if (method != null) {
                method.releaseConnection();
