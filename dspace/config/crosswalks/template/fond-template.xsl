            ...
            <fo:page-sequence master-reference="simpleA4">
                <fo:flow flow-name="xsl-region-body">
                    ...
                    <fo:block font-size="10pt" space-after="5mm" text-align="justify" margin-top="5mm" >
                        <xsl:value-of select="NameAlternative" />
                    </fo:block>
                    ...
                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Soggetto conservatore'" />
                        <xsl:with-param name="value" select="Custodian" />
                    </xsl:call-template>

                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Segnatura del fondo'" />
                        <xsl:with-param name="value" select="Shelfmark" />
                    </xsl:call-template>

                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Numero di documenti'" />
                        <xsl:with-param name="value" select="DocumentQuantity" />
                    </xsl:call-template>

                    <xsl:call-template name="print-values">
                        <xsl:with-param name="label" select="'Nota dell archivista'" />
                        <xsl:with-param name="values" select="Note" />
                    </xsl:call-template>

                    <xsl:call-template name="print-value">
                        <xsl:with-param name="label" select="'Stato di conservazione'" />
                        <xsl:with-param name="value" select="StorageStatus" />
                    </xsl:call-template>
                    ...
                    <xsl:call-template name="print-values">
                        ...
                        <xsl:with-param name="values" select="IdentifierUri" />
                    </xsl:call-template>
