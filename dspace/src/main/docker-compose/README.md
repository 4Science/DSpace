...
## Run DSpace-CRIS 2023_02_x REST from your current branch
```
docker compose -p c23 up -d
```

Finally, initialize the layout using the provided XLS config file
```
docker compose -p c23 -f docker-compose-cli.yml run --rm dspace-cli cris-layout-tool -f /dspace/etc/conftool/cris-layout-configuration.xls
```
...
## Run DSpace-CRIS 2023_02_x REST with a IIIF Image Server from your branch
...
```
docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/docker-compose-iiif.yml up -d
```

## Run DSpace-CRIS 2023_02_x REST and Shibboleth SP (in Apache) from your branch
...
3. Build the Shibboleth container (if you haven't built or pulled it before):
   ```
   cd [dspace-src]
   docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/docker-compose-shibboleth.yml build
   ```

4. Start all containers, passing your public hostname as the `DSPACE_HOSTNAME` environment variable:
   ```
   DSPACE_HOSTNAME=[subdomain].ngrok.io docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/docker-compose-shibboleth.yml up -d
   ```
   ...
   ```
   env DSPACE_HOSTNAME=[subdomain].ngrok.io docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/docker-compose-shibboleth.yml up -d
   ```
...
7. If you want to include Angular UI as well, then you'll need a few extra steps:
      ...
      * Spin up the `dspace-angular` container alongside the others, e.g.
        ```
        DSPACE_HOSTNAME=[subdomain].ngrok.io docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/docker-compose-angular.yml -f dspace/src/main/docker-compose/docker-compose-shibboleth.yml up -d
        ```
...
```
docker compose -p c23 -f docker-compose-cli.yml run --rm dspace-cli create-administrator -e test@test.edu -f admin -l user -p admin -c en
```

Download a Zip file of AIP content and ingest test data
```
docker compose -p c23 -f docker-compose-cli.yml -f dspace/src/main/docker-compose/cli.ingest.yml run --rm dspace-cli
```
...
```
docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/db.entities.yml up -d
```

Download an assetstore from a tar file on the internet.
```
docker compose -p c23 -f docker-compose-cli.yml -f dspace/src/main/docker-compose/cli.assetstore.yml run dspace-cli
```
...
Many DSpace-CRIS configuration settings will reload automatically (after a few seconds).  However, configurations which are cached by DSpace-CRIS (or by Spring Boot) may require you to quickly reboot the Docker containers by running `docker compose -p c23 down` followed by `docker compose -p c23 up -d`.
...
```
docker compose -p c23 -f docker-compose-cli.yml run --rm dspace-cli [command] [parameters]
```
...
```
docker compose -p c23 -f docker-compose-cli.yml run --rm dspace-cli index-discovery -b
```
...
docker compose -p c23 -f docker-compose-cli.yml run --rm dspace-cli dsprop -p dspace.ui.url
```
...
docker compose -p c23 exec -e "db__P__cleanDisabled=false" dspace /dspace/bin/dspace database clean
```
...
4. Delete the `pgdata` volume. WARNING: This deletes all your old PostgreSQL data. Make sure you have that `pgdump.sql` file FIRST!
    ```
    # Assumes you are using `-p c23` which prefixes all volumes with `c23_`
    docker volume rm c23_pgdata
    ```
...
then you MUST change the name/directory in the `db.restore.yml` script.
    ...
    docker compose -p c23 -f docker-compose.yml -f dspace/src/main/docker-compose/db.restore.yml up -d
    ```
