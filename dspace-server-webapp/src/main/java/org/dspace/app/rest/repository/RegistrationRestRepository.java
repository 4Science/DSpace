...
public class RegistrationRestRepository extends DSpaceRestRepository<RegistrationRest, Integer> {
    ...
    public RegistrationRest createAndReturn(Context context) {
        ...
        if (Objects.nonNull(registrationRest.getGroups()) && !registrationRest.getGroups().isEmpty()) {
            try {
     ...
     */
    private void registerEperson(
        Context context, HttpServletRequest request, RegistrationRest registrationRest
    )
